...
let title_text = document.getElementsByClassName('title_text')[0];
// let title_after = getComputedStyle(title_text, ':after').content;
let title_after = getComputedStyle(title_text, ':after');

function title_anime () {
    title_text.classList.remove('drop_down_off');
    title_text.classList.add('drop_down_on');
    let title_move = document.querySelector('.drop_down_on');
    // 타이틀 텍스트 동작 2 (동작 1이 실행 되면 왼쪽 화살표가 나타나서 붙음)
    if (title_move !== null) {
        // title_after.style.color = "red"
